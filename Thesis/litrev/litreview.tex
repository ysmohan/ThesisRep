...
\subparagraph{The recurrent excitation model}

According to this model, LGN excitation triggers a 4 pyramidal cells to fire at the optimum orientation. The layer 4 neuron then recurrently excite other pyramidal cells of the same orientation and the initial orientation signal is amplified. In the non-optimum orientation, the small LGN excitation overlaps with feedforward inhibition and the cell remains silent. The small inhibition then only produces small changes in somatic input conductances and won't be readily observable while recording 
...
\subparagraph{Evidence against recurrent excitation model}

Still relies on geniculate inputs for orientation selectivity.
